...
export const determineStage = (state) => {
    if (state.xp < 500)
        return 'EGG';
    if (state.xp < 2000)
        return 'BABY';
    ...
    if (clean >= speed && clean >= chaos)
        return 'ARCHITECT';
    if (speed >= clean && speed >= chaos)
        return 'HACKER';
    return 'CHAOS';
};
export const feed = (filesChanged, insertions, message) => {
...
export const checkHygiene = (modified, untracked) => {
    ...
    if (hoursSince < 1)
        return; // Don't punish too often
    let healthDrop = 0;
    if (untracked > 5)
        healthDrop += 1 * hoursSince;
    if (modified > 10)
        healthDrop += 2 * hoursSince;
